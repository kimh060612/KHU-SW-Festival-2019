...
class REGRESSION:
    ...
    def Total_REG_LG(self, Importance_name="Importance",  Importance_analye = True, epoch = 1000):
        ...
        params = {'learning_rate': 0.001, 'max_depth': 16, 'boosting': 'gbdt', 'objective': 'regression', 'metric': 'rmse', 'num_leaves': 65, 'feature_fraction': 0.9, 'bagging_fraction': 0.7, 'bagging_freq': 5, 'seed':2018, 'device' : 'gpu'}
    ...
    def Total_REG_CAT(self, Importance_name="Importance", Importance_analye = True, epoch = 1000):
        ...
        for train_index, test_index in cv.split(X_data):
            ...
            self.REGCAT = CatBoostRegressor(iterations=epoch, learning_rate=0.001, depth=4, l2_leaf_reg=20, bootstrap_type='Bernoulli', subsample=0.6, eval_metric='RMSE', metric_period=50, od_type='Iter', od_wait=45, random_seed=17, allow_writing_files=False, task_type="GPU", devices='0')
